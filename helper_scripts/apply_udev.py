...
def parse_interfaces():
    ...
    for line in output:
        if re.match("^.*LOOPBACK.*$", line):
            ...
            mac = cut_line[1]
            if verbose:
                print "interface: " + interface + " index: " + str(ifindex) + " mac: " + mac
            index_map[interface] = {"index": ifindex, "mac": mac}

    if apply:
        for interface in index_map:
            if verbose:
                print "determining driver for interface: " + interface
            success = False
            # Method1
            try:
                ...
                try:
                    driver = subprocess.check_output(
                        ["basename $(readlink /sys/class/net/" + interface +
                            "/device/driver/module) > /dev/null 2>&1"],
                        shell=True).replace("\n", "")
                except subprocess.CalledProcessError, e:
                    try:
                        driver = subprocess.check_output(
                            ["basename $(readlink /sys/class/net/" + interface +
                                "/device/driver) > /dev/null 2>&1"],
                            shell=True).replace("\n", "")
                    except subprocess.CalledProcessError, e:
            ...
            if verbose:
                print "interface: " + interface + " driver: " + driver
    return index_map
...
def delete_rule(search_string):
    ...
    if verbose:
        print ">>> BEFORE"
        show_rules()
    remove_rule = subprocess.check_output(
        ["sed -i '/" + search_string + "/d' " + udev_file], shell=True).split('\n')
    if verbose:
        print "<<< AFTER"
        show_rules()
...
def add_mac_rule(mac,interface):
    ...
    for interface_1 in index_map:
        if index_map[interface_1]['mac'] == mac:
            mac_found = True
    if not mac_found:
    ...
    with open(udev_file, "a") as output_file:
        output_file.write("""ACTION=="add", SUBSYSTEM=="net", ATTR{address}==\"""" + mac +
                          "\", NAME=\"" + interface + "\", SUBSYSTEMS==\"pci\" \n")
    if verbose:
        show_rules()
...
def apply_remap():
    ...
    for interface in index_map:
            ...
            try:
                subprocess.check_call(
                    ["udevadm info -a -p /sys/class/net/" + interface +
                     """ | grep 'SUBSYSTEMS=="pci"' > /dev/null"""], shell=True)
            except subprocess.CalledProcessError, e:
                continue
            lowest_index = index_map[interface]["index"]
            lowest_index_interface = interface
        ...
        if apply:
            if index_map[interface]["driver"] not in drivers:
                drivers[index_map[interface]["driver"]] = True
            print "        driver: " + index_map[interface]["driver"]

    # Leave tunnel and bridge devices alone
    if "tun" in drivers:
        del drivers["tun"]
    if "bridge" in drivers:
        del drivers["bridge"]
    if "vxlan" in drivers:
        del drivers["vxlan"]
    if "bond" in drivers:
...
def main():
    ...
    if args.add:
        add = True
        for mac, interface in args.add:
            additions.append([is_mac(mac), interface])
    if args.delete:
        delete = True
        for mac in args.delete:
            removals.append(is_mac(mac))
    if args.show:
        show = True
    if args.no_vagrant_interface:
        use_vagrant_interface = False
    if args.vagrant_mapping:
        just_vagrant = True
    if args.no_vagrant_default:
        use_vagrant_default = False
    if args.apply:
        apply = True
    ...
    if show:
        ...
        for mac,interface in additions:
            add_mac_rule(mac,interface)
